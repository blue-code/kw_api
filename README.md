...
| `POST`   | `/auth/service-token`             | 서비스 간 인증을 위한 토큰 발급            |     ✅     |
| `POST`   | `/items`                          | 새 아이템 생성                            |     ✅     |
...
**개발/테스트 참고**: 현재 아이템 수정 및 삭제 기능은 개발/테스트 편의를 위해 소유권 확인 로직이 임시로 우회되어 있습니다. 이 변경은 보안 취약점을 발생시키므로, 프로덕션 배포 전 반드시 원상 복구해야 합니다.
